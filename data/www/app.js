...
function setEmergencyHotlineModalOpen(isOpen) {
  const modal = document.getElementById("emergencyHotlinesModal");
  if (!modal) {
    return;
  }
  modal.hidden = !isOpen;
...
async function refreshGpsDiagnostics() {
  const outputEl = document.getElementById("gpsDiagOutput");
  const buttonEl = document.getElementById("gpsDiagRefreshBtn");
  ...
  try {
    const payload = await fetchJson("/gps/diag");
    const lines = [];
    if (payload.recoveryAttempted) {
      lines.push(`Recovery attempt: ${payload.recoveryOk ? "SUCCESS" : "FAILED"}`);
    }
    lines.push(`GPS fix: ${payload.gps?.hasFix ? "YES" : "NO"} | type=${payload.gps?.fixType} | sats=${payload.gps?.satellites}`);
    lines.push(`Coords: ${payload.gps?.latitude}, ${payload.gps?.longitude}`);
    lines.push(`Modem: responsive=${payload.modem?.responsive} simReady=${payload.modem?.simReady} CREG=${payload.modem?.creg} CEREG=${payload.modem?.cereg} CGATT=${payload.modem?.cgatt} RSSI=${payload.modem?.rssi}`);
    lines.push("Raw GPS parser line:");
    lines.push(payload.gps?.raw || "(empty)");
    lines.push("\nAT command responses:");

    (payload.commands || []).forEach((entry) => {
      lines.push(`\n${entry.cmd} -> ${entry.ok ? "OK" : "FAIL"}`);
      lines.push((entry.response || "(no response)").trim() || "(empty)");
    });

    outputEl.textContent = lines.join("\n");
  } catch (error) {
    outputEl.textContent = `Diagnostics error: ${error.message}`;
  } finally {
...
async function tick() {
  try {
    ...
    await Promise.all([refreshGps(), refreshNet(), refreshLogs()]);
    updateTimestamp();
  } catch (error) {
...
const gpsDiagRefreshBtn = document.getElementById("gpsDiagRefreshBtn");
if (gpsDiagRefreshBtn) {
  gpsDiagRefreshBtn.addEventListener("click", refreshGpsDiagnostics);
}

const emergencyHotlinesBtn = document.getElementById("emergencyHotlinesBtn");
const emergencyHotlinesCloseBtn = document.getElementById("emergencyHotlinesCloseBtn");
const emergencyHotlinesModal = document.getElementById("emergencyHotlinesModal");

if (emergencyHotlinesBtn) {
  emergencyHotlinesBtn.addEventListener("click", () => setEmergencyHotlineModalOpen(true));
}

if (emergencyHotlinesCloseBtn) {
  emergencyHotlinesCloseBtn.addEventListener("click", () => setEmergencyHotlineModalOpen(false));
}

if (emergencyHotlinesModal) {
  emergencyHotlinesModal.addEventListener("click", (event) => {
    const target = event.target;
    if (target instanceof Element && target.closest('[data-hotline-close="true"]')) {
      setEmergencyHotlineModalOpen(false);
    }
  });
}
