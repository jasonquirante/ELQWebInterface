...
async function sendSmsFromUi() {
  ...
  const attachLocationEl = document.getElementById("smsAttachLocation");
  ...
  if (!senderNameEl || !senderContactEl || !attachLocationEl || !toEl || !msgEl || !btnEl) {
    return;
  }
  ...
  const includeLocation = attachLocationEl.checked;
  ...
  try {
    const response = await fetch("/sms/send", {
      ...
      body: JSON.stringify({ to, message, senderName, senderContact, includeLocation }),
    });
    ...
    if (!response.ok || !payload.ok) {
      ...
      setSmsStatus(`Template sent to ${payload.to}${suffix}${gpsFallback}`);
      msgEl.value = "";
    }
  } catch (error) {
